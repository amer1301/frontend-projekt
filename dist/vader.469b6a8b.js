...
function initializeMap() {
    ...
    } else // Om kartan redan finns, uppdatera bara vy
    map.setView([
        defaultLat,
        defaultLon
    ], 13); // Uppdatera vy för default plats
...
async function getCoordinates(location) {
    try {
        ...
        if (data.length > 0) {
            ...
            console.log("H\xe4mtade koordinater:", latitude, longitude);
...
async function getWeather(latitude, longitude) {
    ...
    try {
        ...
        console.error("Fel vid h\xe4mtning av v\xe4derdata:", error);
    }
}
...
function displayHourlyWeather(data) {
    ...
    if (times.length > 0 && temperatures.length > 0) for(let i = 0; i < times.length; i++){
        const time = times[i];
        ...
        hourlyWeather.innerHTML = `
                <h4>Tid: ${time}</h4>
                <p>Temperatur: ${temperature}\xb0C</p>
                <p>Regn: ${precip} mm</p>
                <p>Luftfuktighet: ${humidity}%</p>
                <p>Vind: ${windSpeed} m/s</p>
                <p>Sn\xf6fall: ${snowfall} cm</p>
            `;
        hourlyContainer.appendChild(hourlyWeather);
    }
    else hourlyContainer.innerHTML = "<p>Ingen v\xe4derdata tillg\xe4nglig.</p>";
}
function displayDailyWeather(data) {
    ...
    if (days.length > 0 && maxTemperatures.length > 0) for(let i = 0; i < days.length; i++){
        const day = days[i];
        ...
        dailyWeather.innerHTML = `
                <h4>Dag: ${day}</h4>
                <p>Max temperatur: ${maxTemp}\xb0C</p>
                <p>Min temperatur: ${minTemp}\xb0C</p>
                <p>Regn: ${precip} mm</p>
                <p>Sn\xf6fall: ${snowfall} cm</p>
    ...
    else dailyContainer.innerHTML = "<p>Ingen 7-dagars v\xe4derdata tillg\xe4nglig.</p>";
}
// Anropa initializeMap() för att visa kartan direkt
initializeMap();

//# sourceMappingURL=vader.469b6a8b.js.map
